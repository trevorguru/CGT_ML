...
def add_atom_counts(main_dir, df_mag, recalculate, num_ions=10.0):
    """
    ...
    #print(elems_list_set)
    atom_label_list_mag, atom_count_list_mag = get_atom_counts(df_mag, df_elements_mag, 'elem_list', num_ions)
    # above line modified may 4 2019 to include elems_list_set
...
def get_unique_elem_info(main_dir, df, recalculate=False):
    ...
    if not os.path.exists('elements_energies.csv') or recalculate:
        ...
        for ith, cmpd in df['formula'].iteritems():
            ...
            mp_cmpd = mp.core.Composition(cmpd) #updated mar 18, 2021
...
def alloy_testdata_gen(main_dir, CGT, X_atoms, B_atoms, replace_atoms ):
    """
...
def get_atom_counts(df, df_elements, elem_list,num_ions=10.0):
    """
        ...
        # add num_ions 5.4.2019 for differentiate between ABX3 and TMhalides
        # num_ions = 10 for ABX3 and 16 for supercell of TMX
    """
    ...
    for c, cmpd in df[elem_list].iteritems():
        ...
        for ath, atom in enumerate(cmpd):
            #print(atom)
            atomic_ratio = frac_dict[ath][atom]*num_ions*1.0
        ...
        #print(atom_vector.shape, atom_matrix.shape)
...
def Xexists(i, X_val, df_counts, num_ions=16.0):
    """
        find if pair of B atoms exists
        and use to determine whether to pick up energy term
        for Matrix of B atoms and TM atoms
    ...
    for fth, f in enumerate(frac):
        ...
        frac_list.append(atom_count)
    if type(elem_list) == list:
        elem_list_copy = elem_list[:]
    else:
        elem_list_copy = elem_list.copy(deep=True)
    if xval in elem_list_copy:
        xvalexist = True
    else:
        xvalexist = False
    return xvalexist
...
def get_atom_energy_total(df, df_elements):
    ...
    frac_dict_list = []
    for ith, cmpd in df['formula'].iteritems():
        mp_cmpd = mp.core.Composition(cmpd)
...
#import pymatgen as mg
from pymatgen import *  #Lattice, Structure import error mar 17, 2021
from mendeleev import element
...
def df_delta( df_spin_Te, df_nospin_Te, label, elabel1, elabel2, main_dir, num_ions=10.0 ):
    """
    ...
    if 'elem_list' in df_spin_Te.columns: #'elem_list', 'elem_frac',
        ...
        recalculate=True
        #def add_atom_counts(main_dir, df_mag, recalculate, num_ions=10.0):
        df_spin_polariz_Te = add_atom_counts(main_dir, df_spin_polariz_Te, recalculate, num_ions)
...
def gen_cohesive_json(df_input, df_elements):
    """
        calculates cohesive energy from total energy and sum of atomic energies
        - assume data format of theta results
    """
    df = df_input.copy()
    total_elem_energy,elems_list_set,frac_dict_list = get_atom_energy_total(df, df_elements)
    df['total_elem_energy'] = total_elem_energy
    ...
    #cohesive = df['energy'].values - df['total_elem_energy'].values
    #df['cohesive'] = cohesive
    cohesive_spin = df['spin_energy'].values - df['total_elem_energy'].values
    df['cohesive_spin'] = cohesive_spin
    cohesive_afm = df['afm_energy'].values - df['total_elem_energy'].values
    df['cohesive_afm'] = cohesive_afm
    cohesive_spin_so = df['spin_so_energy'].values - df['total_elem_energy'].values
    df['cohesive_spin_so'] = cohesive_spin_so
    cohesive_afm_so = df['afm_so_energy'].values - df['total_elem_energy'].values
    df['cohesive_afm_so'] = cohesive_afm_so
    return df
